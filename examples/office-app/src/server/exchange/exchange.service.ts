...
import {
  normalizeExchangeBinaryForSqlite,
} from "../../workarounds/server/exchange-binary";
...
export function createExchangeService(options: {
  ...
  async function importFile(input: {
    ...
    try {
      const imported = normalizeExchangeBinaryForSqlite(
        await importSnapshot(input.data, input.originalName, unitId, input.type),
      );
    ...
    async importProtocolFile(input: {
      ...
      try {
        if (outputType === 1) {
          ...
          const unitId = randomUUID();
          const imported = normalizeExchangeBinaryForSqlite(
            await importSnapshot(artifact.data, artifact.filename, unitId, input.type),
          );

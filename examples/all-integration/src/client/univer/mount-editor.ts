...
import { installDocContentPermissionsWorkaround } from "../../workarounds/client/doc-content-permissions";
...
export async function mountUniverEditor(
  ...
  registerCollaboration(univer, baseURL, options.unitType);
  registerExchangeAndPrint(univer, options.unitType, baseURL);
  const commentWorkaround = registerComments(univer, options.unitType);
  const docPermissionWorkaround =
    options.unitType === UniverType.UNIVER_DOC
      ? installDocContentPermissionsWorkaround(univer)
      : undefined;
  registerHistory(univer, options.unitType, `${baseURL}/history`, options.container);
  ...
  return {
    ...
    dispose: () => {
      ...
      docPermissionWorkaround?.dispose();
      univer.dispose();
    },

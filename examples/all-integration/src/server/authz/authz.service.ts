...
// SDK 的 Doc/Slide/Board/Base 内容对象复用同一个协议 payload 与应用 ACL contract。
const contentObjects = new Map<
  UnitObject,
  {
    unitType: UniverType;
    field: "documentObject" | "slideObject" | "boardObject" | "baseObject";
  }
>([
  ...[UnitObject.DocumentSection, UnitObject.DocumentParagraph, UnitObject.DocumentEntity].map(
    (type) =>
      [type, { unitType: UniverType.UNIVER_DOC, field: "documentObject" as const }] as const,
  ),
  ...[UnitObject.SlidePage, UnitObject.SlideElement, UnitObject.SlideMaster].map(
    (type) => [type, { unitType: UniverType.UNIVER_SLIDE, field: "slideObject" as const }] as const,
  ),
  [UnitObject.BoardElement, { unitType: UniverType.UNIVER_BOARD, field: "boardObject" }],
  ...[
    ...
    (type) => [type, { unitType: UniverType.UNIVER_BASE, field: "baseObject" as const }] as const,
  ),
]);
...
export function createAuthzService(options: {
  ...
  function readCreateObject(request: ICreateRequest) {
    ...
    const contentObject = contentObjects.get(request.objectType);
    if (contentObject) {
      const payload = request[contentObject.field];
      if (!payload) {
        throw new AuthzError(400, "INVALID_REQUEST", `Missing ${contentObject.field}`);
      }
      return payload;
    }
    throw new AuthzError(
  ...
  return {
    ...
    create(userId: string, request: ICreateRequest) {
      ...
      const expectedUnitType =
        contentObjects.get(request.objectType)?.unitType ?? UniverType.UNIVER_SHEET;
      if (!unit || unit.type !== expectedUnitType) {
        throw new AuthzError(404, "UNIT_NOT_FOUND", "Unit not found or type mismatch");
      }

...
本例使用 Nginx 将请求路由到两个协同服务节点，并提供简单的 Sheet 创建和文件列表，方便观察路由和验证多人协同。你也可以根据现有基础设施，选择其他支持按 unitID 一致性哈希路由的网关或负载均衡方案。
...
跨节点广播组件正在开发中，用于补齐跨节点实时消息投递能力。即使后续版本支持跨节点广播，仍推荐按 unitID 做一致性路由，减少跨节点消息转发与协调开销。

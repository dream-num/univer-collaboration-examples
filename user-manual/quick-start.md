...
- 要搭建自己的服务：继续阅读[搭建协同服务](./integration.md)。
- 要先验证重启后数据仍在：运行 `pnpm example:database-adapter`，并阅读
  [Database Adapter example](../examples/database-adapter/README.md)。
- 要看登录和读写权限：运行 `pnpm example:permissions`，并阅读
  [Permissions example](../examples/permissions/README.md)。
- 要增加版本历史、评论或 Worktree：查看[可选能力](./extensions.md)。

如果两个窗口没有同步，不要先改 OT 或 Service API。按[生产运行](./production.md#按现象定位问题)
